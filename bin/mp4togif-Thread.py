...
def batch_convert_videos_to_gif(input_directory, output_directory, max_workers=4):
    # 确保输出目录存在
...
def process_single_video(input_path, output_path):
    ...
    if not os.path.exists(output_path):
        try:
            ...
            raise RuntimeError(f"{input_path} 转换失败: {str(e)}")
    else:
        print(f'已存在，跳过: {output_path}')

# # 定义一个函数，用于批量转换目录中的所有视频文件为GIF
# def batch_convert_videos_to_gif(input_directory, output_directory):
#     # 确保输出目录存在
#     os.makedirs(output_directory, exist_ok=True)
#
#     # 遍历输入目录中的所有文件
